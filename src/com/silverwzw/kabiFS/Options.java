...
public class Options {
	...
	protected static final Pattern commitNamePattern, branchNamePattern;
	...
	public Options(String ... args) throws ParseException {
		...
		if (i >= 2) {
			if (branchNamePattern.matcher(args[1]).matches()) {
				baseCommitName = args[1] + "@";
			} else if (commitNamePattern.matcher(args[1]).matches()) {
				baseCommitName = args[1];
			} else {
		...
		if (i == 3) {
			if (branchNamePattern.matcher(args[2]).matches()) {
